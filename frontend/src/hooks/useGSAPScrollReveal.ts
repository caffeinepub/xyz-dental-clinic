...
export function useGSAPFadeIn<T extends HTMLElement = HTMLDivElement>(delay?: number) {
  const ref = useRef<T>(null);
...
export function useGSAPSlideUp<T extends HTMLElement = HTMLDivElement>(delay?: number) {
  return useGSAPScrollReveal<T>({ delay });
}

export function useGSAPScaleIn<T extends HTMLElement = HTMLDivElement>(delay?: number) {
  const ref = useRef<T>(null);
